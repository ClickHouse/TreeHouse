...
1. **[`bitTest`](https://clickhouse.com/docs/sql-reference/functions/bit-functions#bitTest)** and **[`bitCount`](https://clickhouse.com/docs/sql-reference/functions/bit-functions#bitCount)**: We treated button combinations as binary integers. `bitTest` allowed us to check if a specific button was pressed in a combination, and `bitCount` instantly gave us the total number of presses (the cost).  
